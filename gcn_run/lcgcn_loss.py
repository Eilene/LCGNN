...
parser.add_argument('--lambda2', type=float, default=1.0, help='z1, z2, lambda')
...
class Net(nn.Module):
    ...
    def forward1(self, data):
        ...
        h0 = torch.softmax(x, dim=1)
        adj_0 = torch.mm(h0, h0.t())
        return F.log_softmax(x, dim=1),adj_0
...
def main():
    ...
    for run in range(RUNS):
        ...
        for epoch in tqdm(range(1, EPOCHES1+1)):
            ...
            model.eval()
            with torch.no_grad():
                logits,adj0 = model.forward1(data)
